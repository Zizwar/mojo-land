...
import { getUserByAccessToken, supabase } from "./mojo.ts";


export default class Mojo {
  addons: [] | undefined;
  tableName: string = "mojos";
  use(addon: any) {
    this.addons = { ...this.addons, ...addon };
  }
  set tableww(name: string = "mojos") {
    this.tableName = name;
  }
  async render(ctx, method) {
    ...
    //check user
    let user = {};
    const accessToken = query("token") || body?.token || cookies.getCookie(ctx, "mojo_token");
    if (accessToken) {
      user = await getUserByAccessToken(accessToken);
      console.log("middle_user=", { user, accessToken });
    }

    const extractColumns = (columns = "") => columns.split(",").map((column: string) => column.trim()) || [];
    const json = (
      data: any,
      status = 200,
    ...
    try {
      ...
      const isAuthorized = () => {
        ...
        const found = user.roles?.some((roleObj: { role: { name: string; }; }) => extractColumnsRoles.includes(roleObj?.role.name)
        );
        return found;
      };
